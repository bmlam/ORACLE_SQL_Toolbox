sta C:\projects\sandbox\toolbox\get_sql_monitor_report.sql 07ypghypadv3d   274 56848 2
...
,s.username, osuser, event
,logon_time ses_sta, status sta
--, s.inst_id 
--, program prg
--, osuser osu
...
;
select * from dba_scheduler_jobs where owner = 'LICENSING'
;
...
--  and s.sid in ( 322, 916 ) and s.inst_id = 1
  and q.sql_id in ( '07ypghypadv3d', 'xx' ) -- on v_tx view 
...
;
-- execution plan 
select * from table( dbms_xplan.display_cursor( sql_id => '55ftavx1us2bz', format=> 'ALL' , CURSOR_CHILD_NO => 0
) )
;
...
-- execution stats 
select child_number chdno, address, sql_id, executions exec#, end_of_fetch_count completed#, rows_processed row_proc, s.* from gv$sql  s
where 1=1
-- and hash_value = 1432471311
--  AND s.address = '000000D8DB4B0C48'
 and sql_id = 'a4fsnwgjj2aa9'
;
select systimestamp, va.statistic#, na.name, va.value from gv$sesstat va join v$statname na ON na.statistic# = va.statistic# where va.inst_id = 2   and va.sid = 213 and va.statistic# in (31,52,139,159,287,548,945) order by 2
;
-- full sql text 
select 
...
where o.object_name = 'SK_DUPLICATION'
;

select se.logon_time, xa.used_urec urec /*, xa.* */from gv$transaction xa join gv$session se on xa.addr =  se.taddr AND xa.inst_id=se.inst_id where 1=1  and se.username = 'DATARC' 
;
...
-- scratch area
select * from t_process_controls 
where run_no= 'B000310087'
;

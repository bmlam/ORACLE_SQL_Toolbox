...
-- now the magic query for time interval unit measured in days
with magic_ as (
...
;
-- now the magic query for time interval unit measured in months
with magic_ as (
    select 'P' as part_name_prefix
        , 'yyyy.mm.dd hh24:mi:ss' as date_mask_universal
        , 'yyyymm' as date_mask_name_infix
        , '' part_name_suffix
        , 1 interval_in_months 
        , to_date('2017.12.01 00:00:00', 'yyyy.mm.dd hh24:mi:ss' ) start_time
        , to_date('2018.04.01 00:00:00', 'yyyy.mm.dd hh24:mi:ss' ) end_time
        , 999 max_partitions 
    from dual
    ...
    select 
      add_months(  m.start_time, (rownum - 1) * m.interval_in_months ) as time_val
    , add_months(  m.start_time, (rownum - 0) * m.interval_in_months ) as time_val_upper_bound
    , m.date_mask_universal, m.part_name_prefix, m.date_mask_name_infix
    , m.part_name_suffix, m.end_time
    from magic_ m
    connect by  level <= max_partitions
